...
author: "Bryan Lu, Youzhi Luo, Linbo Liu, Sijun Tan, Tianhao Wu, Sida Li, Panpan Xu, Anoop Deoras, Ion Stoica"
...
The system architecture driving this hillclimb consists of three components — the *agent* the developer writes, the *runtime* it executes on, and the *trainer* that learns from the agent rollouts. Each exposes clear interfaces that abstract its implementation from the others. Concretely, the setup combines three open pieces:
...
AgentCore Runtime owns the *runtime* — per-rollout sandboxing and elastic scaling — so hundreds of parallel rollouts need no infra management. `rllm-model-gateway` owns the agent↔model boundary, capturing token-level data so the agent never has to handle it. And rLLM owns the *trainer*: its backend flexibility trains a single agent definition against either Tinker (hosted) or veRL (self-managed, distributed) with no agent-side changes. 

![End-to-end architecture: rLLM orchestrates the data flow and launches AgentCore Runtime sessions, which host agent loops. The agent's model client makes requests to rllm-model-gateway, which proxies the inference servers and persists token-level data. rLLM then processes the rollout data and sends it to any training backend of the user's choice.](../assets/agentcore/rllm-agentcore.svg)

Together they serve a larger goal we share with the rLLM team: democratizing reinforcement learning for LLMs — any agent a developer can build and deploy, they should also be able to train. The rest of this post walks through why each of those pieces is necessary, what we contributed to rLLM to make stable long-horizon multi-turn training work, and a deep dive on what the migration agent learned through the RL training process. 


## Why agentic RL needs new infrastructure

Single-turn RL was a relatively light workload for an inference engine. The agent makes one call, gets one completion, the reward model scores it, the trainer steps. The whole loop fits comfortably on the same machine.

A multi-turn coding agent doesn't behave like that at all. A MigrationBench rollout looks like this: the agent reads the repository's `pom.xml`, runs `mvn compile`, gets a stack trace, edits a file, runs `mvn test`, gets a new failure, edits again. Some rollouts converge in five turns. Others run 80+ turns and take **30 minutes** of wall-clock time. 
